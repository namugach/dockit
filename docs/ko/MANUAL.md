...
기존 Dockit 프로젝트를 복제하여 유사한 환경을 가진 새 프로젝트를 생성합니다. 이 명령어는 Docker 이미지와 설정을 포함하여 프로젝트 설정을 복제하는 과정을 간소화합니다.
...
이 명령어는 `dockit`으로 생성된 프로젝트와 더 이상 관련이 없는 Docker 네트워크와 볼륨을 찾아 제거합니다. 이를 통해 불필요하게 디스크 공간을 차지하는 "좀비" 리소스를 안전하게 정리할 수 있습니다.
