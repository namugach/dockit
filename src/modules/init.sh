...
display_current_settings() {
    ...
    echo -e "$MSG_PASSWORD: ${GREEN}${USER_PASSWORD:-${PASSWORD:-$DEFAULT_PASSWORD}}${NC}"
    echo -e "$MSG_WORKDIR: ${GREEN}${WORKDIR:-${WORKDIR:-$DEFAULT_WORKDIR}}${NC}"
...
set_default_values() {
    ...
    USER_PASSWORD=${USER_PASSWORD:-${PASSWORD:-$DEFAULT_PASSWORD}}
    WORKDIR=${WORKDIR:-${WORKDIR:-$DEFAULT_WORKDIR}}
...
get_custom_values() {
    ...
    read -p "$MSG_INPUT_PASSWORD [${USER_PASSWORD:-${PASSWORD:-$DEFAULT_PASSWORD}}]: " input
    USER_PASSWORD=${input:-${USER_PASSWORD:-${PASSWORD:-$DEFAULT_PASSWORD}}}
    
    read -p "$MSG_INPUT_WORKDIR [${WORKDIR:-${WORKDIR:-$DEFAULT_WORKDIR}}]: " input
    WORKDIR=${input:-${WORKDIR:-${WORKDIR:-$DEFAULT_WORKDIR}}}
