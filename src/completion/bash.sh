...
_dockit_completion() {
    ...
    local commands="init start build up stop down connect status help version migrate setup run join ps list image"
    ...
    if [ "$COMP_CWORD" -eq 1 ]; then
        ...
        COMPREPLY=( $(compgen -W "list remove prune clean" -- ${cur}) )
        return 0
    fi
}
